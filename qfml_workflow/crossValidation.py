...
class PurgedKFold(_BaseKFold):
    ...
    def split(self,X,y=None,groups=None):
        ...
        #if len(X.loc[self.t1.index].index) != len(self.t1):
            raise ValueError('X and ThruDateValues must have the same index')
        indices=np.arange(X.shape[0])
        mbrg=int(X.shape[0]*self.pctEmbargo)
        test_starts=[(i[0],i[-1]+1) for i in \
        #test_starts=[(i[0],i[-1]) for i in \
...
def cvScoreCustomPipe(clf,X,y,sample_weight, pipeSampleParam,scoring='neg_log_loss',t1=None,cv=None,cvGen=None,
            pctEmbargo=None):
    if scoring not in ['neg_log_loss','accuracy']:
        raise Exception('wrong scoring method.')
    from sklearn.metrics import log_loss,accuracy_score
    ...
    score=[]
    for train,test in cvGen.split(X=X):
        fit_params = {pipeSampleParam:sample_weight.iloc[train].values}
        fit=clf.fit(X=X.iloc[train,:],y=y.iloc[train],
                    **fit_params)
        if scoring=='neg_log_loss':
            prob=fit.predict_proba(X.iloc[test,:])
